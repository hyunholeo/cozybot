...
client.on('ready', () => {
  console.log('켰다.');
  client.user.setPresence({ game: { name: '무요' }, status: 'online' })
});
...
class forbidden {
   ...
   */
  constructor(words) {
    if (!Array.isArray(words)) throw new Error("Words 인자는 반드시 배열이여야합니다.")
    this.words = words
  }
  checkWords(message) {
    if (message.attachments.size !== 0) return
    if (message.content === "" || message.content === undefined) throw new Error("message 인자는 공백이여서는 안됩니다.")
    let regExp = /[\{\}\[\]\/?.,;:|\)*~`!^\-_+<>@\#$%&\\\=\(\'\"^(0-9)]/gi
    let string = message.content.replace(regExp, "")
    this.findOfWord(message, string, this.words)
  }

  findOfWord(message, string, words, catchWord = "", loop = false) {
    ...
    for (let i = 0; i < words.length; i++) {
      ...
      for (const content of text) {
        if (words[i] === content) {
          catchWord = `"${content}"`
          text.splice(text.indexOf(words[i]), 1, catchWord)
          message.delete().then((msg) => {
            msg.channel.send("감지된 욕설: " + words[i] + "\n전체 문장: `" + text.join(" ") + "`")
          })
          loop = true
          break
        } else {
...
const Discord = require("discord.js")
const client = new Discord.Client()
const badWord = new forbidden(["시x", "병x", "개새x"]) // 단어 추가는 "단어1", "단어2", ... 이런식으로 합니다.
const Token = "482551923828981760"
